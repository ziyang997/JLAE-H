...
def test_model_all(prediction, test_mask, train_mask):
    ...
    print('ndcg_1     \t[%.7f],\t||\t ndcg_5     \t[%.7f],\t||\t ndcg_10     \t[%.7f],\t||\t ndcg_15     \t[%.7f]' \
          % (ndcg[0],
             ...
             ndcg[3]))
    return ndcg[2]
   # return precision, recall, f_score, ndcg[2]
...
def test_all_u(sess, model):
    Decoder = sess.run(model.rating_u, {model.users: list(range(model.num_users)), model.input_R_U: model.train_R_U})
    Decoder = np.array(Decoder)
    Decoder = np.delete(Decoder, -1, axis=1)
    test_model_all(Decoder, model.test_R, model.train_R)

def test_all_i(sess, model):
    Decoder = sess.run(model.rating_i, {model.items: list(range(model.num_items)), model.input_R_I: model.train_R})
    Decoder = np.array(Decoder)
    Decoder = np.delete(Decoder, -1, axis=1)
    test_model_all(Decoder.T, model.test_R, model.train_R)
def test_all(sess, model):
    Decoder = sess.run(model.Decoder, {model.users: list(range(model.num_users)), model.input_R_U: model.train_R,
